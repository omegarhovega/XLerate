...
Private Sub lstDependents_Click()
    ...
    selectedText = lstDependents.List(lstDependents.ListIndex, 0)
...
Private Sub lstDependents_KeyDown(ByVal KeyCode As MSForms.ReturnInteger, ByVal Shift As Integer)
    ...
    ElseIf KeyCode = vbKeyReturn Then
        ...
        If lstDependents.ListIndex >= 0 Then
            selectedText = lstDependents.List(lstDependents.ListIndex, 0)
            NavigateToCell selectedText
        End If
    ElseIf KeyCode = vbKeyRight Then
        ...
        If lstDependents.ListIndex >= 0 Then
            currentIndex = lstDependents.ListIndex
            selectedText = lstDependents.List(currentIndex, 0)
...
Private Sub StoreOriginalData()
    ...
    For i = 0 To lstDependents.ListCount - 1
        ...
        dataItem = lstDependents.List(i, 0) & "|" & _
...
Private Sub ParseListBoxToTree()
    ...
    For i = 0 To lstDependents.ListCount - 1
        ...
        ' Auto-expand level 0 (root) and level 1 nodes to show first level of dependents immediately
...
Private Sub RefreshTreeAndRestoreIndex(targetIndex As Integer)
    ...
    If targetIndex >= 0 And targetIndex < lstDependents.ListCount Then
        lstDependents.ListIndex = targetIndex
    ElseIf lstDependents.ListCount > 0 Then
        ' If target index is out of bounds, select the last available item
        lstDependents.ListIndex = lstDependents.ListCount - 1
    End If
End Sub
...
Private Sub UpdateColumnWidths(newInnerWidth As Single)
    ...
    lstDependents.ColumnWidths = Int(addressWidth) & ";" & Int(valueWidth) & ";" & Int(formulaWidth)
...
Private Sub RepositionHeaders()
    ...
    With Me.Controls("lblHeader2")
        .Left = lstDependents.Left + addressWidth + FORM_PADDING
        .Width = Int(valueWidth)
    End With
    
    With Me.Controls("lblHeader3")
        .Left = lstDependents.Left + addressWidth + valueWidth + FORM_PADDING
...
Private Sub LoadSavedDimensions()
    ...
    savedWidth = GetSetting("XLerate", "frmDependents", "Width", MinWidth)
    savedHeight = GetSetting("XLerate", "frmDependents", "Height", MinHeight)
...
Private Sub SaveCurrentDimensions()
    ...
    If Me.Width > 0 And Me.Height > 0 Then
        SaveSetting "XLerate", "frmDependents", "Width", Me.Width
        SaveSetting "XLerate", "frmDependents", "Height", Me.Height
    End If
